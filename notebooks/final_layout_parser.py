...
Original file is located at
    https://colab.research.google.com/drive/10MRNyc1dRP8IhKhTKQc9YyRuLJdOGmSR
...
def append_to_faiss(urs_name, urs_section_id, block_count, conn, cursor, batch_id):
    """
    Appends new content blocks to a FAISS index for a specific URS document.

    Args:
        urs_name (str): Name of the URS document.
        urs_section_id (int): Section ID of the URS document.
        block_count (int): Number of content blocks to process.
        conn (psycopg2.Connection): Database connection.
        cursor (psycopg2.Cursor): Database cursor.
        batch_id (int): Batch ID for filtering content blocks.
    """

    # Initialize FAISS index and its storage path
    faiss_index, faiss_index_path = initialize_embedding_tools(urs_name)

    try:
        # Fetch the urs_id using the hashed URS name
        cursor.execute("SELECT id FROM urs WHERE urs_hash = %s",
                       (hashlib.md5(preprocess_name_for_hash(urs_name).encode()).hexdigest(),))
        urs_id_row = cursor.fetchone()

        if not urs_id_row:
            raise ValueError(f"No urs_id found for urs_name: {urs_name}")
        urs_id = urs_id_row[0]

        # Fetch unindexed content blocks using urs_id and batch_id
        cursor.execute("""
            ...
            WHERE usm.urs_id = %s
            AND cb.faiss_index_id IS NULL
            AND cb.batch_run_id = %s
        """, (urs_id, batch_id))
        rows = cursor.fetchall()

        if rows:
            # Prepare query strings with section names
            query_strings = [f"{row[2]}||{row[1]}" for row in rows]

            # Generate embeddings using sentence transformer
            embeddings = sentence_model.encode(query_strings, convert_to_numpy=True)

            # Store current FAISS index size
            pre_size = faiss_index.ntotal
            faiss_index.add(embeddings)
            post_size = faiss_index.ntotal
            print(f"FAISS index size before: {pre_size}, after: {post_size}")

            # Compute FAISS index positions
            index_positions = list(range(pre_size, post_size))

            # Prepare update values (faiss_index_id, content_block_id)
            update_values = [(idx, row[0]) for idx, row in zip(index_positions, rows)]

            # Execute batch update to store FAISS index IDs in the database
            execute_batch(cursor, "UPDATE content_blocks SET faiss_index_id = %s WHERE id = %s", update_values)
            print(f"Batch {batch_id}: Updated {len(update_values)} content blocks with FAISS index IDs")
            ...
            print(f"Batch {batch_id}: Successfully saved FAISS index to {faiss_index_path}")

        else:
            print(f"No unindexed content blocks found for urs_id {urs_id} (urs_name: {urs_name}) and batch_id {batch_id}")

    except Exception as e:
        # Roll back transaction on error
        conn.rollback()
        print(f"Error during FAISS update for batch {batch_id}: {e}")
        raise
