...
# ============================================================================
# LAMBDA HANDLER ADAPTERS (for API Gateway VPC Link integration)
# ============================================================================

def lambda_event_from_flask_request(request):
...
def flask_response_from_lambda_response(lambda_response):
    ...
    if isinstance(lambda_response, dict):
        ...
        response = jsonify(body_data) if isinstance(body_data, dict) else body
        for key, value in headers.items():
            response.headers[key] = value
        return response, status_code
    else:
        return jsonify({'error': 'Invalid Lambda response format'}), 500

@app.route('/presigned-url', methods=['GET'])
def presigned_url_endpoint():
    """Presigned URL endpoint - calls Lambda handler"""
    try:
        import sys
        sys.path.insert(0, '/var/task/lambda')
        from presigned_url import lambda_handler
        import types
        ...
        event = lambda_event_from_flask_request(request)
        
        # Call Lambda handler
        lambda_response = lambda_handler(event, context)
        
        # Convert Lambda response to Flask response
        return flask_response_from_lambda_response(lambda_response)
    except Exception as e:
...
def knowledge_base_endpoint():
    """Knowledge base endpoint - calls Lambda handler"""
    ...
    try:
        import sys
        sys.path.insert(0, '/var/task/lambda')
        from knowledge_base import lambda_handler
        import types
...
def delete_document_endpoint():
    """Delete document endpoint - calls Lambda handler"""
    if request.method == 'OPTIONS':
        return '', 200, {
            'Access-Control-Allow-Origin': '*',
            'Access-Control-Allow-Methods': 'OPTIONS,POST',
    ...
    try:
        import sys
        sys.path.insert(0, '/var/task/lambda')
        from document_deleter import lambda_handler
        import types
...
def rag_query_endpoint():
    ...
    # Proxy to existing /query endpoint (which already handles RAG queries)
    return query()

@app.route('/rag-query-multimodal', methods=['POST', 'OPTIONS'])
def rag_query_multimodal_endpoint():
